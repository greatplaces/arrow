...
UniValue rescanblockchain(const UniValue& params, bool fHelp)
{
    ...
    if (!params[1].isNull()) {
        ...
        else if (pindexStop < pindexStart) {
    ...
    int stopBlock = pwalletMain->ScanForWalletTransactions(pindexStart, pindexStop, true);
    if (!stopBlock) {
      //        if (pwalletMain->IsAbortingRescan()) {
      //    throw JSONRPCError(RPC_MISC_ERROR, "Rescan aborted.");
      // }
        // if we got a nullptr returned, ScanForWalletTransactions did rescan up to the requested stopindex
      stopBlock = chainActive.Height();
    }
    else {
    ...
    response.pushKV("stop_height", stopBlock);
        return response;
}
